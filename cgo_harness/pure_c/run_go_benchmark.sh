...
ensure_checkout "$ORACLE_RUNTIME_REPO" "$ORACLE_RUNTIME_COMMIT" "$RUNTIME_DIR"
ensure_checkout "$ORACLE_GRAMMAR_REPO" "$ORACLE_GRAMMAR_COMMIT" "$GRAMMAR_DIR"
...
if [[ -n "$SOURCE_PATH" ]]; then
  ...
  deep_sha="$("$artifact" --dump "$SOURCE_PATH" | sha256sum | awk '{print $1}')"
else
