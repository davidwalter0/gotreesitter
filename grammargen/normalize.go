...
func classifyRules(g *Grammar) (tokens, nonterms []string) {
	...
	for _, name := range g.RuleOrder {
		...
		if isTerminalRule(rule) {
			...
			if isBareStringOnlyChoice || isSharedHiddenStringToken || isSharedVisibleStringToken || (isVisible && isBareString && !namedPlainStringRules[name]) || (isBareString && sharedStrings[terminalStringValue(rule)]) {
