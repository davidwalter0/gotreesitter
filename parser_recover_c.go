...
// CRecoveryGateDiagnostics summarizes generated-table recovery eligibility.
type CRecoveryGateDiagnostics struct {
	Supported bool
	Reason    string
}

// CertifyCRecoveryCostCompetition validates a generated language's recovery
// surface and updates the default-on metadata used by the runtime gate.
func CertifyCRecoveryCostCompetition(lang *Language) CRecoveryGateDiagnostics {
	diag := DiagnoseCRecoveryGate(lang)
	if lang == nil {
		return diag
	}
	lang.CRecoveryCostCompetitionCapable = diag.Supported
	lang.CRecoveryCostCompetitionEnabledByDefault = false
	if diag.Supported && generatedCRecoveryDefaultSafe(lang) {
		lang.CRecoveryCostCompetitionEnabledByDefault = true
	}
	return diag
}

// CertifyGeneratedCRecoveryCostCompetition is kept for generated grammar call
// sites that predate the shorter helper name.
...
// DiagnoseCRecoveryGate checks for the generic parser.c recovery surface in
// generated tables: a recover row, action entries, and usable tables.
func DiagnoseCRecoveryGate(lang *Language) CRecoveryGateDiagnostics {
	...
	if !lang.GeneratedByGrammargen {
		return CRecoveryGateDiagnostics{Reason: "not generated by grammargen"}
	}
	if lang.StateCount == 0 || len(lang.LexModes) == 0 {
		return CRecoveryGateDiagnostics{Reason: "missing parser states or lex modes"}
	}
	if len(lang.ParseActions) == 0 {
		return CRecoveryGateDiagnostics{Reason: "missing parse actions"}
	}
	return CRecoveryGateDiagnostics{Supported: true}
}
...
func generatedCRecoveryDefaultSafe(lang *Language) bool {
	...
	if len(lang.ExternalSymbols) == 0 && lang.ExternalTokenCount == 0 {
		return true
	}
	return lang.ExternalScanner != nil && len(lang.ExternalLexStates) > 0
}
